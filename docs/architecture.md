...
- **结算**（含杠杆，见下面「杠杆与爆仓」）：
  `payoutUnits = max(0, floor(投入 + 名义本金 × (平仓价/开仓价 − 1) − 名义本金 × 平仓价/开仓价 × 费率))`，
  其中 `名义本金 = 投入 × 杠杆`。**杠杆 = 1 时它逐位退回加杠杆之前的算式**
  `floor(投入 × 平仓价/开仓价 × (1 − 费率))` —— 存量仓位与那几条钉死的数都靠这条恒等。
  `floor` 是刻意的 —— 舍入永远朝系统一侧，宁可少发一个单位也不凭空多铸。手续费
  **只在平仓侧收一次**（开仓免费、持有免费），且按**平仓时的名义本金**收（杠杆越高摩擦
  越大，这是杠杆最自然的一道刹车）。最小投入 1 条鱼干。
  **存储精度是这套结算的关键参数**：`floor` 每次都朝系统丢零头，而零头的上界就是
  存储的最小刻度。刻度还是 0.1 条时，投 0.1 条（= 1 个单位）的仓位只要价格不涨过
  0.1%（那个阈值就是当时的费率）就必然结算成 0 —— 那不是高风险而是几乎必赔的陷阱。
  2026-09 把刻度提到 0.0001 条（迁移 `21_fish_units_1e4`）之后零头上界降到 0.0001 条，
  **同一笔结算少丢 1000 倍**：投 1 条、涨 0.05% 的一笔损耗 0.0003 条（费率 0.02% 占
  0.0002，`floor` 只吃掉 0.0001）。
...
- **结算公式只有一份实现**：`src/lib/market-math.ts` 的 `settleClose` —— 服务端真结算
  ...
  ⚠️ `leverage` 在这个函数的入参里是**必填**，刻意不给默认值：一个忘了传杠杆的调用点
  会静默地把 10 倍仓按 1 倍结算，而屏幕上那个数看起来完全合理。
...
- **★ 「借来的钱」是账外的，因此没有任何借贷表 ★** 它就是上面那个「无限水池」的另一种
  ...
  去建 `loans` / `margin` 之类的表，也别在 `users` 里虚构系统账户行（见本节开头那条）。
- **★ 爆仓不是加在借贷之上的一道安全措施，它是账本形态逼出来的唯一形态 ★**
  权益在「开仓价 × (1 − 1/杠杆)」处归零，再跌下去公式给出负数 —— 而**负数在这个账本里
  没有表示法**（`postEntry` 只收非零整数、`users.driedFish` 不许为负、余额必须等于流水
  之和）。于是 `max(0, …)` 截断：用户亏光投入、一分不多欠。
  ⚠️ **手动平仓与强制平仓走同一个 `max(0, …)`**，所以价格穿过爆仓价之后两条路给出
  **同一个数**（都是 0）。这不是巧合，是必需的：若手动平能亏穿，账本当场撕裂；若强平
  能多留一点给用户，理性策略就变成「别平，等它爆我」—— 那是个鼓励用户等待的产品。
- **爆仓价在开仓那一刻算一次就写死**（`market_positions.liquidation_price`，
  算法是 `market-math.ts` 的 `liquidationPrice`）。存而不重算的理由与 `payout_units`
  同源：有争议时以库里的为准，而且**强平引擎的判据必须与用户屏幕上那一行是同一个数**
  —— 两层各算一遍，症状就是「按用户看到的价不该爆、系统却爆了」。1 倍仓恒为 0，
  那正是「1 倍永不被强平」的字面意思（价格到不了 0 以下），页面对它显示「—」。
- **★ 强平引擎是本站第一个「自己动手动钱」的后台循环 ★**（`market-liquidator.ts`，
  第四个后台循环，`MARKET_LIQUIDATE_MS` 默认 15 秒、`0` 可关）。前三个循环要么只投递、
  要么只写展示缓存；这一个会在用户没按任何按钮时改仓位行的状态。因此：
  - **它不写鱼干流水**。爆仓价处的权益恰好为 0，实发恒为 0 —— 没有钱动过，也就不该有
    流水（同「实发为 0 的平仓也不写流水」那一档）。**不变式由
    `tests/unit/market-math.test.ts` 的「以爆仓价结算，实发恒为 0」钉着**：
    谁把 `liquidationPrice()` 改成含维持保证金（爆仓时还剩一点权益），那条会当场变红
    —— 那一刻必须回头给强平补一条流水。
  - **结算价是爆仓价，不是触发时的现价**（写进 `exit_price` 的是那一列）。两条理由：
    ① **封顶** —— 按现价结算遇到插针会算出负权益，只能截成 0；按爆仓价结算则让「亏光」
    是**算出来的**而不是**截出来的**。② **时间无关** —— 爆仓价是常量，所以「什么时候
    发现」不影响「结算成多少」，引擎晚一轮、挂十分钟再起来，用户拿到的数完全一样。
    ⚠️ 推论：**两次 tick 之间的插针不触发爆仓**（跌下去又弹回来，那一轮扫不到就不爆）。
    这不是漏洞：用户无法影响 tick 时机，最坏是被白送一次「幸免」；而按第 ② 条，
    它也绝不会让任何人多亏。
  - **判定用现取价**（`fetchQuote()`，与成交同一条边界），理由与成交那条**不同**：
    不是为了防套利（用户按不了这个按钮），而是**判定必须诚实** —— 拿 15 秒前的缓存价
    去爆一个人的仓，会在他价格已经弹回来之后把他强平掉。行情源挂了就跳过这一轮，
    **不降级、不猜**。仓位留着，下一轮再说。
  - **它不判禁言**：禁言是「不能说话」，不该顺带变成「不能止损」。被禁言的人手上还开着
    的杠杆仓照旧会被爆（也能自己平掉）。新增这条写路径时最容易顺手补一个 `!isMuted`
    —— 别加（同 `sell` 那条，见下面的禁言小节）。
  - ⚠️ **它不 import `market-service`**（那边 import 本模块取闸门判据，反向成环会炸在
    模块求值上）。要复用的算术全在零依赖的 `market-math`。它算实发时费率传 0 ——
    那不是「爆仓不收手续费」，而是**在问权益、不问到手**（权益都是 0 了，扣不扣都一样）。
  - ⚠️ **那个告警的判据是「实发是否为 0」，不是「权益是否为 0」**：浮点下爆仓价处的
    权益会留下 ~1.8e-12 的尘埃（实测 3×/5×/10× 都有，2× 恰好干净），拿权益判会让告警
    **每次强平都响**。而实发是 floor 过的整数，尘埃过不了 floor 那道坎（实测恒为 0）。
- **★ 关掉强平 = 同时关掉杠杆开仓 ★** `openPosition` 问的是 `isLiquidationRunning()`
  （引擎**真的起来了**才算，不是「env 配了」），引擎没在跑时杠杆开仓一律 503，
  而 **1 倍照旧**（它永远碰不到爆仓价，不需要引擎）。卖一个兑现不了的产品比不卖更糟。
  页面上那排档位同款处理：只留 1 倍可用 + 一句「强平服务未运行」——
  与「行情拉不到就禁掉买入」同一档，**不是**「入口跟着藏」（那条红线针对的是档位不够）。
  ⚠️ **闸门在幂等重放之后、限频之前**：重放只是回读既有仓位、不产生新风险，不该因为
  引擎此刻没跑就失败；而一次注定被拒的请求没道理烧掉调用方的额度。
- **平仓的重放判据是 `status !== 'open'`，不是 `status === 'closed'`**：爆仓也是结清。
  写成后者的话，一个已经爆掉的仓位会走进取价 + 结算 + 条件写那一段 —— 条件写会挡住它
  （`where status='open'` 匹配 0 行）→ 落到 `MarketAlreadyClosedError` → 回读，
  **结果碰巧还是对的**。这类「结果碰巧对」的错法最难发现，所以判据必须写全。
  路由据此把文案从「已卖出」改成「已爆仓」（`liquidated: true`）—— 用户可能是
  「看到跌穿了、点卖出」才发现早就爆了，回一句「已卖出，−100 条」会让他以为是自己卖掉的。
- **杠杆不进开仓的幂等键**（`openKey` 仍只吃 user/symbol/units）：同一个键重放时回报的是
  **当初那一笔的真实倍数**（从库里读）。所以带着不同 leverage 重试同一个键不会买成两笔、
  也不会改掉已成交那笔的倍数。**别去「补」它** —— 把杠杆塞进键的派生，同一笔重试就会
  变成第二笔。
- **页面上两个数分得很清**：买入弹窗里的「参考爆仓价」是按**展示价**估的（真实那条线
  要等成交价出来才定，文案明说了）；持仓行里的「爆仓」读的是**那一行写死的列**，
  不自己乘一遍。杠杆选择器只管**下一笔**，与已有持仓无关 —— 所以每一行自己带一枚倍数角标。
- **一行 = 一个批次（lot）**，不是聚合持仓。每次开仓插一行，结清时就地改成 `closed`
  （本人平）或 `liquidated`（被强平）—— 两者都是终态，读取方问的是「还是 `open` 吗」。
...
- **幂等**：开仓靠 `market_positions.open_key` 的**唯一约束**（每次唯一；调用方给了客户端键
  ...
  「近乎归零」变成 500。**强平同样不写流水**（实发恒为 0，见上面的杠杆小节），
