...
> ⚠️ **Turnstile 会挡住自助注册。** 站点若启用了人机验证，脚本无法自行注册 ——
> 需要有人在浏览器里手工建号一次。建完号再让机器人用密码登录即可，
> 之后不再需要 Turnstile。
...
- ❌ **不支持 `<img>`** —— 白名单里没有。贴图必须走 `image_id`（§9.2）
...
- **`chatDaily` 每天 800 条是硬上限。** 一个「看到消息就回」的机器人很容易烧光，
