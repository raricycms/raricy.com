...
| `fish webhook-rekey` | 把存量回调签名密钥搬到专用加密密钥（迁移期一次性，可重复跑） |
| `fish liquidate` | 立刻扫一轮练手盘的杠杆仓强平（不等定时器） |
...
**关于 `fish webhook-rekey`**：回调签名密钥是加密落库的。搬之前，加密钥匙由
`SECRET_KEY`（会话 JWT 的签名密钥）派生 —— 换一次会话密钥，商户就再也收不到回调，
而站点这边只在投递日志里看到失败。这条命令把存量密文重封到专用钥匙
`FISH_ENCRYPTION_KEY` 上（**只换封装、不换密钥本身**：商户手里那把签名密钥不变，
验签代码一行都不用改）。`.env` 里同时有这两把钥匙时跑一次即可，跑完 `SECRET_KEY`
就只是会话密钥了。**逐行判状态**（已搬过的原样跳过），所以中断、部分完成、
或「当时解不开后来补上了钥匙」都能靠再跑一次收敛；两把钥匙都解不开的行它不动，
会列出来要求人工查证。`npm run diagnose` 段 4 报的就是「还剩几条没搬」。

**关于 `fish liquidate`**：练手盘的杠杆仓有一个**强平引擎**在跑（常驻后台循环，默认
每 15 秒扫一轮，见架构 §6.13）。这条命令是它的手动那一份 —— 引擎被关掉过一段时间、
或刚重启完想立刻补一轮时用。判断、结算价、副作用与引擎**逐条相同**：现价穿过那行上
存着的**爆仓价**就结清，结算价用爆仓价（持户亏光投入、不会亏穿），**不写鱼干流水**
（实发恒为 0）。也可以与定时器同时跑，不会重复结清。
⚠️ 它**不可撤销**（只把仓位置成 `liquidated`，没有撤销路径），而且引擎被关掉往往
正是因为行情源不可信 —— 那时手工跑等于拿一份可能有问题的价去结清别人的仓位。

`amount` 是正整数，单位是**整个小鱼干**。⚠️ 这是 **CLI 刻意收窄的**，别当成全站口径 ——
网页与 API 那一侧收最多 4 位小数（鱼干在库内的最小刻度是 0.0001，见
`docs/architecture.md` §6.3）。运维发钱按整数走是为了少一个手滑的小数点。写路径就是**一次本地事务**（余额与流水同事务
