...
- 终端里跑会先弹出「即将执行」确认屏（列出目标用户、角色变化、后果），答 `y` 才真的执行。
- 管道 / CI 等非交互场景必须显式加 `--yes`，否则直接拒绝 —— **CLI 绝不会挂在 stdin 上等输入**。
- 每次变更都会写一条审计日志（`change_role`，默认公开可见，见 `/audit` 公示页）。

审计主体默认取**库内最早的站长**，可用 `--as <username>` 显式指定。主体必须是一个真实用户：
`admin_action_logs.admin_id` 是指向 `users.id` 的外键，不能用伪造 ID —— 所以库内一个站长都没有时，
需要审计的命令会直接报错而不是静默跳过。

两个由此而来的行为约束：

- **不能修改自己的角色**（`setRole` 的既有规则）。站点只有一个站长时，他不能用 CLI 把自己降级 ——
  要么 `--as` 指定另一个站长，要么先加一个。
- 站点若**还没有任何站长**，先用 `--as <username>` 指定一个真实用户来完成第一次任命。

⚠️ 跑 CLI 的机器仍是权限边界：这些命令不校验「执行者本身是不是站长」（权限分档在服务层，
见 `src/lib/admin-user-service.ts` 的 `setRole`）。生产环境务必把跑该命令的服务器 shell 列入堡垒机白名单。

## 二、小鱼干（fish）

对应账户微服务的复式记账。**写路径 fail-closed**：远端账户服务失败 → 本地写入被补偿事务精确撤销（对用户等价于回滚）→ 退出码 2。绝不静默成功。
