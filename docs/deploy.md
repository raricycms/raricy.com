...
| **中文字体** | **必须有**（任意含 CJK 的字体，见下） | 画报 / 收款码是服务端用 sharp（librsvg + fontconfig）光栅化的，**没有中文字体时画报上的字全是豆腐块**。二维码不受影响（矢量矩形），所以图能生成、也能扫 —— 只有字是方框，属于「半坏」状态，最容易漏掉。见 `npm run diagnose` 段 5「画报中文字体」 |
