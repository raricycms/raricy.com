...
>
> ⚠️ **头像框素材不适用上面这段** —— 它 2026-09 起随代码入库、住 `public/static/frames/`，
> 不再是这个还原流程要处理的东西（原先它也在 `instance/frames/`，由该脚本建目录）。
> 理由：那些图是**我们自己画的**（源码 = `scripts/make-frame-demos.mjs`），与第三方表情
> 不是一类；而「部署时手工拷一份」曾是一个**没有报错**的步骤，漏拷 = 全站静默不显示框。
> 老机器上残留的 `instance/frames/` 已经没人读了，可以删。自查用
> `npm run cli -- frame list --keys`（见 `docs/cli.md`），守护见
> `tests/unit/frame-assets.test.ts`。
