...
`/fish/trade` 是除讨论区之外**第二个多栏工作台**（自选 | 图表 | 下单 + 持仓），
...
| 视口 | 布局 | 中间那列的宽度 |
|------|------|----------------|
| ≥1200 | 三栏 `200px \| 1fr \| 300px` | 容器 1140 → ~608px |
| ≤1199 | 三栏收窄 `168px \| 1fr \| 264px` | 容器 960 → ~496px（自选这一档隐藏走势线） |
| ≤991 | 自选变**横向条**横跨整行，下面「图 \| 右栏」两列 | — |
| ≤767 | 单列：自选条 → 图 → 右栏两卡 | — |

- **`minmax(0, 1fr)` 不是可选的**：不写它，列宽就是 min-content —— 图里的 SVG 与轴标签
  会把列撑开，窄屏直接出横向滚动条。同理网格里的 SVG 要**显式给高度**，
  否则 `height: 100%` 会塌成 0。
- **窄屏是同一份 DOM**：自选列表靠 CSS 从竖排摊成横条，不另写一套 JSX
  （两套 DOM = 两份状态 + 两处会 drift 的类名）。
...
十字光标、价/时徽标全是 HTML，用百分比定位叠在**同一格网格**上（`.trade-chart` 是 grid，
...
- **「强平引擎没在跑」时那几档是灰的（`disabled` + `opacity: .35`），不是隐藏的** ——
  ...
  见 `docs/architecture.md` §6.13 的杠杆小节）。这不是「入口跟着藏」那条红线：1 倍照旧能买。
- **持仓行里的倍数角标（`.trade-position__lev`）是徽标不是按钮**（淡底 + `$radius-small`），
  **绝不能套 `btn-tab`** —— 套了就会被读成「点它能改倍数」，而上方那个选择器只作用于
  **下一笔**。同一个理由：`--danger` 与 `--error` 在 `.trade-field__hint` 上是两个类名，
  虽然此刻同色（「你填错了」vs「这个数会吃掉你的本金」）。
...
**为什么不能写 @media**：持仓卡所在那一栏的宽度**不是视口的单调函数** ——
...
想用 @media 表达「卡窄了就折行」得拼出一串**不相连的区间**，还得跟着 `.trade-layout`
的列宽改动一起维护。
...
- ⚠️ **折行之后 main 必须跨满两列**（`grid-column: 1 / -1`）。只折行、让 main 与盈亏
  挤在第一行的话，那 252px 里 main 只剩 ~114px，那一行照样碎成三行 ——「折了但没治好」。
- ⚠️ **阈值 380px 是量出来的**：行内三样加两条间隙需要 ~331px（main 150 + 盈亏 90 +
  卖出 59 + 2×16），再加这一行自己的左右内距 32px。改列宽或往行里加内容时回来重量一次。
- **老浏览器不支持容器查询时这条规则整块丢掉，落回单行版式**（即与修复前一样）——
  所以它不需要 `@supports` 降级。这也是它**不违背** §6.8 那条「避开 `:has()`」的理由：
  当年避开是因为滑块几何**丢了就坏**，这里丢了只是少折一次行。
- 几何用例：`tests/e2e/fish-trade-layout.spec.ts`（三档各量一次首列宽度 + 开仓那一行的
  行数，外加一条「卡最宽那档真的不折行」）。
...
（黄脸还有第三档：**整条正文只有它一张**时用 `:not(.rich-emoji-solo)` 让压缩那条不匹配、
