...
> 会话失效（改密、被强制下线、被降权）后一律 `401 请先登录` ——
> 当成「重新登录」的信号，而不是重试。
...
>
> ⚠️ **校验顺序**：先判「有没有签到」→ 再判「是不是已经翻过」（是则幂等成功）
> → 再判牌池 → 最后判 `chosenIndex`。所以**已经翻过牌的用户带非法 index 再来，
> 拿到的是现值而不是 `400 无效的选择`**。机器人不必为此写特例分支。
...
`GET /api/fish/leaderboard` —— **免登录**，返回排行榜（按 `total_fortune` 等）。
它只读、与签到接口是两回事，口径见站点排行榜页面。
