...
> ⚠️ **`503` 之后可以重试**：账户服务不可用时本地写入已被补偿回滚，
> 你仍处于「已签到、待翻牌」的状态，**重新翻一次即可**（牌还是那副牌）。
> 那笔没发生的鱼干不会凭空出现。
...
### 7.3 失败一定是 fail-closed

翻牌是鱼干写路径。远端账户服务不可用时，本地写入会被**补偿回滚**，
接口回 `503` —— 绝不会出现「本地记了账、远端没记」且无人知晓的状态。

> 📌 对机器人的含义：**看到 `503` 就重试一次**（前面说过，牌还在）。
> 但别把重试写成循环 —— 服务端已经明确告诉你这次一分钱都没动。
...
| `503` | 账户服务暂时不可用，本地已回滚 —— **重试一次**即可（§5.1） |
