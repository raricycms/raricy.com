...
> 📌 公开收藏夹还有一条**出图**的读口：`GET /api/poster/favorite/<6 位句柄>` 返回分享用的
> 二维码 PNG（需 core+ 登录、按 `posterMinute` 30 次/分/人限频）。它读的是同一份公开数据，
> 只是产物是图不是 JSON。
...
需要一个 **core+ 账号 + 会话 cookie**。本站的**会话**不用 Bearer（它是 HttpOnly
Cookie），也没有专用的
...
> ⚠️ 示例里的 `-b jar.txt` 不能省 —— 这条接口**要 core+ 登录**，不带 cookie 是 `401`。
...
| **增 / 减条目** | **300 次 / 小时、1500 次 / 天** | §9.4 —— 与文章点赞**同额**，但另起一对桶 |

超限一律 `429 操作过于频繁，请稍后再试`。

> ⚠️ **「加条目」是吃配额的**（`fav:item:h:/d:` 两个桶）—— 一个「把几百篇一次性收进
> 收藏夹」的机器人会在这里撞 429，而不是无限额。**改标题、删收藏夹才不吃配额。**
