...
> ⚠️ **文章默认「仅站内可见」（`internal`）。** 想让站外的人（未登录访客）也读得到，
...
| 分享卡片 `GET /api/og/blog/:id` | 一张 PNG，**免登录**；`link` / `public` 都回 200，`internal` / 已软删 / 不存在**三者同形 404** |
...
- ⚠️ **取值必须恰好是三档之一，空串也 `400`** —— 它刻意**不**套用 POST / PUT 那套
  「空串 = 没传 = internal」的归一化（§7.1）：那条规则是给「旧客户端压根没提这件事」用的，
  而在这条窄接口上显式传空串几乎一定是写错了，静默当成「改成站内」会让人在文章
  从公网消失之后才发现。
...
- **对外公开是不可逆的。** `visibility: "public"` 之后，搜索引擎与第三方存档
  可能已经抓走副本 —— 改回 `internal` 收不回那些副本，社交平台的卡片缓存也要
