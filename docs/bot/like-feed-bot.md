...
| 文章不存在 / 已被软删 | `404 文章不存在` |
| 账户服务不可用 | `503 账户服务暂不可用，投喂失败，请稍后再试` |

> ⚠️ **`amount` 只收整数。** `1.0` 可以（它就是 1），`0.5` 不行；
> 鱼干本身可以有小数（别人转给你的分成余额常常是 `3.8` 这样），
> 但**投喂的数量必须是整数**。

> ⚠️ **`503` 是安全的失败**：本地写入已被补偿回滚，等于这笔没发生。
> **可以重试一次** —— 但重试前建议先查余额（`POST /api/fish/market/balance`，
> 见 `docs/bot/fish-bot.md` §3.2），确认刚才那笔真的没扣。
>
> ⚠️ **不要为了「确保成功」重复提交**：这个接口**没有**幂等键参数，
> 每次成功返回 200 都是一笔真实投喂。
...
> （设计意图见 `docs/architecture.md` §6.5）。
...
| `503 账户服务暂不可用，投喂失败` | 本地已回滚，那笔没发生 —— 可以重试**一次** |
...
if (bal.balance >= 5) {
  ...
  const out = await r.json();
  console.log(r.status, out.message, out.balance);   // 503 → 可以重试一次
}
```
