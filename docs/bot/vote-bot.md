...
> 会话失效（改密、被强制下线、被禁言 / 重置密码）后所有接口一律回 `401 请先登录`。
> 把 `401` 当成「重新登录」的信号，而不是重试。
>
> ⚠️ **被降权（core → user）不属于会话失效** —— 会话仍有效，只是投票接口改回
...
站内**四处**正文支持内容引用语法（博客正文 / 云剪贴板正文 / 评论区 / 讨论区），
其中会把 9 位 ID 渲染成投票嵌入的**只有前两处**：
...
> 要么把投票引用写进**博客正文或剪贴板正文**。写在评论或讨论里只会显示成一串记号。
>
