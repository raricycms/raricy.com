...
| **登出**（作废当前会话） | `POST /api/auth/logout` | §9.1 |
...
| **改别人的任何东西** | ❌ | ❌ | 机器人（core 档）没有任何「代操作」接口。改角色 / 禁言 / 重置别人密码那几条在 `/api/admin/*`，是管理员专属，不在本文范围 |
...
> ⚠️ **`401` 的文案不统一，别去匹配文案**：`/api/users/me`、`/api/auth/change-password`、
> `/api/auth/authentic` 回的是 **`401 未登录`**；通知那几条回 **`401 请先登录`**；
> 而 `GET /api/notifications/stream`（SSE）回的是一段**纯文本** `Unauthorized`
> —— 根本不是 JSON。**判据一律是状态码 `401`。**
...
`GET /api/users/alice` 同样有效 —— 讨论区与评论区的「用户名片」`[@用户/alice]` 就是
按名字取资料的（见 `../guide/内容引用语法指南.md`）。但**按名字查要 core+**：

| 句柄 | 谁查得了 | 查不到 |
|------|----------|--------|
| 用户 id（UUID） | **所有人**（含游客） | `404 用户不存在` |
| 用户名 | **core+**（非 core / 游客 → `403 需要核心用户权限`） | `404 用户不存在` |

> 为什么按名字要收档、按 id 不收：id 是 UUID，只能从页面链接里捡；**用户名到处都是、
> 可以拿来枚举**。本站对可猜的句柄一律收档（6 位收藏夹、8 位剪贴板同理），而名片本来
> 也只有 core+ 发得出来。
>
> 📌 查不到时一律 `404`，**不按查看者改答案** —— 档位不够不代表「这个人不存在」。
>
> ⚠️ 用户名**大小写敏感**（`alice` 与 `Alice` 是两个不同的账号），且不做归一化。
...
```json
{ "code": 200, "message": "密码修改成功，请使用新密码重新登录。", "redirect_url": "/login" }
```
...
> 📌 所有字段都会先 `trim()`。**改密这条路径没有长度上限** —— `100` 字符那条上限
> 只在注册时判；这里只有下限（8 位）与「不能与原密码相同」。
...
没有请求体。成功 `{ "code": 200, "message": "已退出登录" }`，并清掉会话 cookie。

> ⚠️ **登出只有 POST 这一条路**（没有 `GET /api/auth/logout`，这是刻意的：
> GET 会被浏览器预取、被第三方页面上的 `<img>` 触发 —— 那种「看一眼什么就被登出」
> 的故障本站真出过）。
>
> 📌 机器人下线时用它作废自己的会话，比留着等 30 天过期干净。
