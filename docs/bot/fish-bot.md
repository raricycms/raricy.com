...
curl -X POST https://raricy.com/api/fish/market/balance \
  -H 'Authorization: Bearer raricy_xxxxxxxxxxxxxxxxxxxxxxxx' \
  -H 'Content-Type: application/json' \
  -d '{}'
```
...
| 不能做的事 | 转账、收银台支付、用户搜索 —— 带着令牌打这些接口会拿到 **403**（不是 401） |
...
- **只读凭据**（§2.1.1）：每次请求只查一次库，120 次/分，且**只能读**；
...
- **不给** → 页面每次加载都会生成一个新的支付标识，于是**用户付完刷新再付一次就是真的第二笔**。
  这种重复只能靠你自己对账发现并退款，而退款是退不回来的（§9 末尾）。
