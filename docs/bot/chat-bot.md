...
读游标 —— 它决定了两件事：§9.3 里的 `unread_count`，以及顶栏「讨论」小红点熄不熄。响应：
...
4. 它**不碰站内通知** —— 别人 @ 你产生的那些通知条目是另一套（`/api/notifications/*`），
   读讨论不会自动把它们标成已读。
...
**不在表里的三个接口**（不消耗任何配额，但也别拿它们当心跳刷）：

| 接口 | 为什么没有配额 |
|------|----------------|
| 正在输入（§7.5） | 只有一条「同频道 3 秒一次」的服务端节流 |
| 已读（§7.6） | 只是推进游标 —— 但留意它顺带决定顶栏「讨论」红点熄不熄 |
