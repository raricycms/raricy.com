...
> 拿不到一个能用的直链。
>
> ⚠️ **单文件闸与配额闸都按「原始字节」判**（第 3、4 步都在压缩之前）——
> 一张 11 MB 的 PNG **会被直接拒掉**（`文件过大，单文件上限 10 MB`），
> 别指望「压缩后会变小」能救它。
>
> 📌 只有**已用配额的累加**记的是**压缩后**的大小（落盘成功之后才 `used += saved.fileSize`），
> 所以配额消耗会比你传进去的字节少。但那是入库之后的账，拦不住超限的那一张。
>
