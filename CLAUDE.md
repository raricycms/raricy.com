...
- **分层：一份房间层 + 每款棋一份纯规则**
  - 共用：`board-shared.ts`（协议）· `board-room.ts`（房间注册表与权威判定）·
    `game-bus.ts`（SSE 订阅）· `api/game/_shared.ts`（闸门 + 错误映射 + **12 个 handler
    工厂**，60 条路由的实现都在那里，各 `route.ts` 只是「import + 一行赋值」）。
  ...
  - 各自：`<game>-rules.ts`（零依赖纯规则，前后端同一份）+ `<game>-room.ts`（薄门面）
    + 棋盘组件 + 10 条路由。走子类棋**不需要适配器**（棋盘本身就满足 `RoomBoard`）。
...
- **席位是点出来的，不是排出来的**：进房只落**观战台**（`spectators`），坐哪一席由本人
  点 —— `takeSeat(seat)`（`playing` → `inGame`；那一席有人 → `seatTaken`）。点到**空着的**
  另一席就是换先（原席位空出）；与对手互换座位不做。`leaveSeat` 退回观战台。
  `waiting → playing` 的迁移**只在 `waiting` 时发生**（否则已分胜负的房间会被坐回
  `playing`，赢家还在、棋盘却重新能走）。
- **非对局中，掉线即释放席位**（对局中只标记 + 60s 判胜；`settle` 时补一次，因为掉线是
  边沿触发的）。由此得到不变量：**`playing` ⇒ 两席都有人**，对局进行中新人偷不走座位。
  代价是「切标签页」也算断开，所以**建流首帧要带 `you`**（`RoomStreamEvent`）：客户端
  据此发现"我不在座位上了"，自动坐回原席 —— 少这一帧，被释放过的人会一直以为自己还坐着。
- **悔棋要对手同意**，撤到自己上一次走子之前：`undoPlies(turn, seat)` 给出 1 或 2 步
  （**前后端同一份**），对手同意才回退。请求**不冻结棋局**（走一手即作废，见 `playMove`），
  所以它既没有定时器、也不会把房间卡在"等回应"上。`checkStack` 压的是**走子前**的 check
  （撤回后重算不出来），长度即 `plyCount`；**`requestRematch` 换棋盘时必须清空它** ——
  不清的话第二局第一手会按上一局的步数撤，`room.turn` 与棋盘内部的 turn 就此错开，
  双方永久 `illegalMove`。
- **房间状态里带 `id`**（席位与观战台名单都有）：头像走 `/api/avatar/<id>`，「· 你」也靠
  它认人。所以那条「快照不泄露 userId」的老规矩已经改成「**只给 id 与显示名**，别的
  一律不给」——`tests/service/gomoku-room.test.ts` 的隐私用例钉着字段集合。
...
- **规则模块零依赖**，禁 import 任何 server-only：前端与服务端跑同一份，不存在两份判定。
  ...
  - 五子棋是**先手禁手**（Renju 口径，2026-09 从 free-style 改过来）：黑棋的三三 /
    四四 / 长连是**禁手点，走不上去**（拒绝落子，不是走完判负）；白棋无禁手，长连
    照样算胜；五连优先于一切禁手。**改口径要同时动 `gomoku-rules.ts` 的注释、
    `docs/guide/gomoku-online.md` 与 `tests/unit/gomoku-renju.test.ts`。**
    井字棋是三格整线（3×3 上 `>= 3` 与 `== 3` 等价，别照抄那边的 `>=` 扫描）。
