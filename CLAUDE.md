...
- 玩具区菜单分**单机 / 联机**两段（`/game` 一个页面，不分路由）。同时有两种模式的游戏
  ...
  井字棋只有联机一种玩法，故只出现在联机区。
- **联机棋类 = 一份共用房间层 + 每款棋一份纯规则**：
  - 共用：`board-shared.ts`（协议）· `board-room.ts`（房间注册表与权威判定）·
    `game-bus.ts`（SSE 订阅）· `api/game/_shared.ts`（闸门与错误映射）·
    客户端 `useOnlineRoom.ts` + `OnlineRoomPanel.tsx`。
  - 各自：`<game>-rules.ts`（零依赖纯规则，前后端同一份）+ `<game>-room.ts`（薄门面，
    只把棋盘绑上去）+ `Online<Game>.tsx`（棋盘渲染与文案）+ 8 条路由。
  - **加第三款棋时不该动 `board-room.ts`**；发现要动，先判断是棋类共性还是这款棋的特性。
- **两种棋共用一张房号表**（房号全局唯一，房间带 `kind`）。每条操作都要声明期望的
...
- **规则模块零依赖**（`gomoku-rules.ts` / `tictactoe-rules.ts`），禁 import 任何
  server-only：前端与服务端跑同一份，不存在两份判定。五子棋长连（6 子以上）也算胜、
  井字棋是三格整线（3×3 上 `>= 3` 与 `== 3` 等价，别照抄那边的 `>=` 扫描）。
- **联机棋类的 CSS 外壳是 `.board-*`**（`_board.scss`），不是各游戏自己的前缀 ——
  单机五子棋、联机五子棋、联机井字棋三处共用一套按钮 / 状态行 / 席位栏 / 房号条。
