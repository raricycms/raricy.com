...
//   ① 档位分叉。正文搜索是全站最贵的读（一次请求 = count + findMany 两次全表 LIKE
//      扫描，正文约 48.6MB），只对 core+ 开放。**必须明确报错而不是静默忽略** ——
//      静默忽略会让调用方以为搜了正文，那是本仓库红线紧盯的「静默失效」。
...
describe('GET /api/blogs — search_fields 的档位与白名单', () => {
  ...
  it('匿名带 search_fields=content → 401（明确报错，不静默忽略）', async () => {
    const res = await get('?search=x&search_fields=content');
    expect(res.status, '静默忽略会让调用方以为搜了正文').toBe(401);
  });

  it('非 core 带 search_fields=content → 403', async () => {
    const u = await makeUser({ role: 'user' });
    await login(u.id);
    expect((await get('?search=x&search_fields=content')).status).toBe(403);
  });

  it('core 带 search_fields=content → 200，搜得到正文独有词且带片段', async () => {
  ...
  it('不传 search_fields → 与改动前一致：匿名 200、且搜不到正文里的词', async () => {
    const u = await makeUser({ role: 'core' });
    await makeBlog({ authorId: u.id, title: '标题无关', content: '正文里有恐龙化石' });

    const res = await get('?search=恐龙化石');
    expect(res.status).toBe(200); // 匿名照常可用
    const body = (await res.json()) as { blogs: unknown[] };
    expect(body.blogs, '默认字段集不含正文').toHaveLength(0);
  });

  it('search_fields 不含 content 时不设档：匿名也能按标题/简介搜', async () => {
    const u = await makeUser({ role: 'core' });
  ...
  it('core 搜正文超过 blogSearchMinute 额度 → 429', async () => {
    const u = await makeUser({ role: 'core' });
