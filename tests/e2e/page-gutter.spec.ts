...
test('抬头与首屏内容在窄屏左右都有檐沟（不贴屏幕边）', async ({ page }) => {
  ...
  for (const { url, targets } of PAGES) {
    ...
    for (const sel of targets) {
      ...
      if (left < MIN_GUTTER - 0.5 || right < MIN_GUTTER - 0.5) {
        bad.push(
          `${url} ${sel}：left=${left.toFixed(1)} right=${right.toFixed(1)}（需 ≥ ${MIN_GUTTER}）`
        );
      }
    }
  }
  ...
  expect(
    bad,
    '这些东西左右贴到了屏幕边 —— 页边距归容器，抬头带里的内容要自己套一层 .container'
  ).toEqual([]);
});
