...
//   （来源是联系页那个黑方块）。其余类名交给 tests/unit/css-tsx-classes.test.ts：
//   那边全量扫 .tsx/.ts，无样式却要用的类名逐个登记理由。本文件留着不并过去，
//   是因为「黑方块」的报错比那边的通用文案更直指病灶。
//
// ⚠️ 早先这里写着「其余类名不查，因为项目里有大量 JS 钩子类（.article-checkbox、
//   .toggle-featured）和纯语义包装」——那两个类名其实全仓无人消费，是被误当成
//   JS 钩子的死类名（已从组件里删掉）。「它是个钩子」不能靠注释断言，
//   得由新守卫的 CONSUMED 那条去验。
//
