...
const GUARD_SYMBOLS = [
  ...
  // 调用方拿不到会话也能读，但只读得到 link / public 档的文章，internal / 已软删 /
...
const PUBLIC_READ_ROUTES: Record<string, string> = {
  ...
  // 头像框素材字节。与表情同性质：不属于任何账号、不随会话变化。
  // ⚠️ 到期**不在这里判**（也不该判）—— 到期的是一条**引用**（谁在戴），
  // 不是这些字节：一个已过期的用户和一个从没拥有过的人拿到的是同一张 PNG。
  '/api/frames/[key]': '头像框素材字节：站点素材，非用户数据；到期影响引用不影响字节',
