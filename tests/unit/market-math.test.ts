...
//   • 涨一倍 → 1.998 条（e2e tests/e2e/fish-trade.spec.ts 也钉着这个数 ——
//     这里钉算式，那里钉「现取的价真的被用上了」）
//   • 平价卖出**也要亏**：亏的正好是手续费。「买进去立刻卖是负和」就是这一条，
//     也是「宁可少发也不多铸」在用户侧的可见后果
//   • 涨跌幅与盈亏率差一个费率 —— 弹窗里是两个数，别让它们悄悄变成同一个
//
// ⚠️ 这几个数字跟着存储精度走：精度从 0.1 条抬到 0.0001 条之后，floor 少丢的零头让
// 涨一倍的实发从 1.9 变成 1.998。再改精度时这几处必红（同 e2e 文件头那条提醒）。
...
describe('settleClose（平仓结算）', () => {
  it('平价卖出：实发只少一个手续费；涨跌幅 0%，盈亏率 −0.1%', () => {
    const s = settle(ENTRY);
    expect(s.grossUnits).toBe(10000);
    expect(s.payoutUnits).toBe(9990); // 1.0000 − 0.0010
    expect(s.feeUnits).toBeCloseTo(10, 6);
    expect(s.changePercent).toBe(0);
    expect(s.profitPercent).toBeCloseTo(-0.1, 6);
  });

  it('涨一倍：floor(10000 × 2 × 0.999) = 19980 个单位 = 1.998 条', () => {
    const s = settle(ENTRY * 2);
    expect(s.payoutUnits).toBe(19980);
    expect(s.changePercent).toBeCloseTo(100, 6);
    expect(s.profitPercent).toBeCloseTo(99.8, 6);
  });

  it('腰斩：floor(10000 × 0.5 × 0.999) = 4995 个单位 = 0.4995 条', () => {
    const s = settle(ENTRY / 2);
    expect(s.payoutUnits).toBe(4995);
    expect(s.changePercent).toBeCloseTo(-50, 6);
    expect(s.profitPercent).toBeCloseTo(-50.05, 6);
  });
  ...
  it('费率乘在**未 floor 的毛额**上，全程只 floor 一次', () => {
    ...
    // 正确：floor(1001.9 × 0.999) = floor(1000.8981) = 1000
    expect(s.payoutUnits).toBe(1000);
    // 若先把毛额 floor 掉再乘费率：1001 × 0.999 = 999.999 → 少发 1 个单位。
  ...
  it('盈亏率与涨跌幅是两个数，差的正好是手续费那一截', () => {
    // 涨 12.5% 的一笔：到手比行情少 0.1 个点（费率），这就是「频繁进出被磨」
