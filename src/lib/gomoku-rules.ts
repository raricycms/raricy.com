...
/**
 * 轮到谁走 —— **由着法历史推出**（走完一手就轮到对方；空盘由先手开局）。
 *
 * 【为什么不是"走一手翻一次面"】两者只在一个地方分岔：**终结比赛的那一手**。
 * 走完一手要翻面，而获胜 / 判和的那一手后面没人接了，回合就停在落子方。于是靠翻面
 * 攒出来的「当前回合」在终局后是**过时**的，悔棋时再翻一次就把它还给了对手 ——
 * 表现是同一方连下两步（先手方刚连成五子、悔棋之后白棋又走了一手）。
 * 从历史重算没有这个前提：撤掉谁的一手，就轮到谁走。
 */
export function sideToMove(history: Move[]): Player {
  return history.length > 0 ? otherOf(history[history.length - 1].player) : BLACK;
}
