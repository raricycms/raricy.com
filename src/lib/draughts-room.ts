// ─────────────────────────────────────────────────────────────────────────────
// draughts-room.ts — 国际跳棋联机的房间层（薄封装）
//
// 【实现全在 board-room.ts】房间的开/关、席位、观战、掉线判胜、TTL 回收、revision
// 语义对**所有**联机棋类都一样，所以只写一份；本文件只负责把 国际跳棋的棋盘
// （DraughtsBoard）绑上去。
//
// 【想改房间行为？改 board-room.ts】别在这里加逻辑 —— 另外四款棋走的是同一份代码，
// 在这里加一分歧，其它游戏就静默地少了那个行为。
//
// 【走子类棋不需要适配器】DraughtsBoard 本身就满足 RoomBoard（rows / cols / grid /
...
// 棋子颜色。国际跳棋**白先**，所以 `black` 席执白、`white` 席执黑。
//
// 【棋盘规则不在这里】见 draughts-rules.ts，前端与服务端共用同一份。
// ─────────────────────────────────────────────────────────────────────────────

import type { GameDefinition, RoomBoard } from './board-room';
import { DraughtsBoard } from './draughts-rules';
...
/** 仅供测试：清空国际跳棋的房间（不动其它棋的）。 */
export const __resetDraughtsRooms = api.__reset;
/** 仅供测试：当前国际跳棋房间数。 */
