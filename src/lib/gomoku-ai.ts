...
// 四局全部下到满盘和棋，执白反而四局全胜）。
//
// 实测战绩（每手 2 万节点、先后手各半、8 局）：普通档 **8:0** 胜旧 AI（那个「旧 AI」
// 是 Flask 时代那套，与本文档后面 A/B 里说的「旧版」不是一回事）。
//
...
interface Params {
   ...
   *
   * 【2026-09 复测：问题不在「算错」，在「贵而少用」】先查过一遍它是否踩着排序
   * 层那个 bug（「对手价值恒为 0」，见 `orderScore`）——**没有**：VCT 的攻方着法
   * 直接来自 `scanThreats` 的 `meFour`/`meThree`，按扫描顺序试，压根不走 `orderScore`。
   * 实现逐行读过，守方解招集（活四成点 ∪ 反冲四点）与 Allis 的判据一致，没有会漏
   * 解招的地方。
   *
   * 于是量了它的命中率与开销（18 个中盘局面、3 秒档）：
   *   · 命中率 **1/18**，而那一手杀 **d4 就能找到（0ms）** —— 搜索自己就看得见
   *   · 失败时每次烧 ~800ms（3 秒档的 27%），而失败是常态
   * 对局对照也一致：普通@3s+VCT 打「改动前的普通@1s」30 局 13:17（43.3%）；
   * 把 VCT 限到 6 层、快路份额压到 6% 也只有 14:16（46.7%）。都在 50% 噪声内。
   * 而且那 30 局里 3 秒方的完成深度 5.47 对 1 秒方的 4.42 —— **搜得更深却没赢**
   * （「时间买不到棋力」这条结论在本版本上依然成立）。
   */
  useVct: boolean;
}
 ...
 *
 * 【2026-09 加先手禁手之后重测了这一组 —— 结论变了，但没有定论】同一个对照
 * （「普通@1s」对「普通@3s + VCT」，也就是被删掉的「困难」档）在禁手规则下：
 *
 *     120 局（30 开局 × 两色 × 两批），困难档得 **58.3%**
 *     按开局聚类 95% CI **48.6% ~ 68.1%** —— **跨过 50%，分辨不出**
 *     （自由规则时代那一轮是 50.0%，60 局，CI 37.3 ~ 62.7）
 *
 * 点估计从 50% 抬到了 58%，颜色拆开也一致（困难档执黑 73.3%、执白 43.3%，
 * 两边都高过对手）。**但这不足以说「困难档在禁手下变得有意义了」** —— 这个
 * 设计里同一开局的几局高度相关，真正的标准误约 ±9.8pp 而不是二项口径的
 * ±4.5pp，要分辨 ~8pp 的差得把样本加到四倍左右（约 500 局）。
 * **别拿 58.3% 去论证恢复困难档**，要恢复就先补样本。
 *
