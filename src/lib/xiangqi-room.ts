// ─────────────────────────────────────────────────────────────────────────────
// xiangqi-room.ts — 中国象棋联机的房间层（薄封装）
//
// 【实现全在 board-room.ts】房间的开/关、席位、观战、掉线判胜、TTL 回收、revision
// 语义对**所有**联机棋类都一样，所以只写一份；本文件只负责把 中国象棋的棋盘
// （XiangqiBoard）绑上去。
//
// 【想改房间行为？改 board-room.ts】别在这里加逻辑 —— 另外四款棋走的是同一份代码，
// 在这里加一分歧，其它游戏就静默地少了那个行为。
//
// 【走子类棋不需要适配器】XiangqiBoard 本身就满足 RoomBoard（rows / cols / grid /
...
// 棋子颜色。象棋**红先**，所以 `black` 席执红、`white` 席执黑 —— 组件里那张
// 映射表（SEAT_LABEL）负责把席位上色成红/黑，本层不掺和。
//
// 【棋盘规则不在这里】见 xiangqi-rules.ts，前端与服务端共用同一份。
// ─────────────────────────────────────────────────────────────────────────────

import type { GameDefinition, RoomBoard } from './board-room';
import { XiangqiBoard } from './xiangqi-rules';
...
/** 仅供测试：清空中国象棋的房间（不动其它棋的）。 */
export const __resetXiangqiRooms = api.__reset;
/** 仅供测试：当前中国象棋房间数。 */
