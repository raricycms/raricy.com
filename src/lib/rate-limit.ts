...
export const RULES = {
  ...
  /**
   * 鱼干练手盘（/fish/trade）的开仓与平仓。
   * 【它防的不是刷屏，是出站流量 + 远端写入】每笔成交都要**现取一次行情**
   *（一次真实出站 HTTP，见 market-price.ts）外加**一次远端账户服务转账**。
   * 不封顶的话，一个脚本就能拿本站当代理去刷行情源、顺手把账户服务的写队列灌满。
   * 【为什么比转账松】转账是 30/时 —— 它在给别人发通知（防的是骚扰）；练手盘
   * 谁也不打扰。20/分与 300/天对正常看盘的人绰绰有余，对脚本则是一道硬墙。
   * 桶键：trade:h:{用户 id} / trade:d:{用户 id} —— 前缀自带。
   * **别复用 transfer:**（rule 不参与分桶，共用前缀会让两条路互相吃额度）。
   */
  tradeMinute: { limit: 20, windowMs: 60 * 1000 },
  tradeDaily: { limit: 300, windowMs: 24 * 60 * 60 * 1000 },
  /**
   * 鱼干商城的**租头像框**（/api/fish/market/rent）。
   *
   * 【为什么有】它是第二类「用户可以反复自助触发、每次都要写库」的鱼干消费
   *（第一类是转账）。每次租用是一个事务里的三笔写（扣鱼干 + 流水 + 持有行），
   * 而余额固然是花销的天花板，**它却不是写次数的天花板** —— 一个手上攒了几百条
   * 鱼的脚本可以拿 1 天 × N 次把 user_frames 的到期时刻推到几年后，
   * 顺手刷出几百行流水。这两个上界都该有。
   * 【为什么给得比转账松】转账 30/时是因为它在给别人发通知（防骚扰）；
   * 租框谁也不打扰，30/时 + 200/天对正常人是一辈子碰不到的数。
   * 桶键：framerent:h:{用户 id} / framerend:d:{用户 id} —— 前缀自带。
   * **别复用 transfer:**（rule 不参与分桶，共用前缀会让两条路互相吃额度）。
   *
   * ⚠️ **这两条没有对应地写进 `docs/bot/`** —— 与全站其它配额不同，那不是漏了：
   *    `/api/fish/market/rent` **只认会话**（站外脚本调不到，理由见该 route.ts
   *    的文件头），所以「租金」不是对外契约，站外读者那边没有可复述的东西。
   *    做限频审计的人别把它当成缺口补一份文档。
   */
  frameRentHourly: { limit: 30, windowMs: 60 * 60 * 1000 },
  frameRentDaily: { limit: 200, windowMs: 24 * 60 * 60 * 1000 },
  /**
   ...
   * （另两条读类配额 chatPoll / spiderFavoritePerIp 防的是请求数量，不是单请求体量。）
   *
   * 【谁在消耗】两条路径共用这一条配额，因为它们开销相同：
   *   · `/blog?search=` 页面（requireCoreUser 之后）
   *   · `GET /api/blogs?...&search_fields=content`（route 里做了 core+ 校验）
   * 共用是刻意的 —— 各配一条等于额度翻倍。
   *
