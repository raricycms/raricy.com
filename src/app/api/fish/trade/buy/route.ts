...
// 所以页面（/fish/trade）与这里、以及 sell 三处各判一次（页面与接口必须同档）。
//
// 【不需要 core+ 的页面入口照样渲染】顶栏与 /fish 面板不对任何人藏入口，档不够的
// 用户点进来是 403（与签到、讨论一致，见 CLAUDE.md「入口不跟着藏」）。
//
// 【禁言只挡这一处】三处判定**不对称**：buy 判禁言（禁言不开新仓），sell 与 quote
// 只判档位（已开的仓必须能出）。理由见 sell/route.ts 头部 —— 别为了「三处一样」
// 把禁言判定补到那边去，那等于让禁言变成锁仓。
//
