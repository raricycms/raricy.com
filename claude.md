...
| `pages/` | 页面特有样式 | 不放可复用样式 |
| `themes/` | 主题（暗色等） | 可选 |
| `vendors/` | 第三方样式覆写 | 不改动第三方原文件 |
...
**main.scss 引入顺序：** abstracts → base → layout → components → utilities → pages → themes → vendors
...
只有 blog 模块有完整分层（views → services → validators → utils）。其他模块（clipboard、game、tool、vote、photowall、checkin）路由多写在 `__init__.py`，服务层更简单，无独立验证器目录。写新模块时参考 blog 的分层结构。

\#### 文档

[docs/](docs/) 目录含 9 个参考文档：项目概述与技术架构、全站限额与频控汇总、部署与运行、前端样式构建指南、内容引用语法指南、云剪贴板使用指南、cattca-guide、cattca-syntax、故事模块。
