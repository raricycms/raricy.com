...
\- 小鱼干管理：flask fish grant <username> <amount> / flask fish deduct <username> <amount> / flask fish balance <username> / flask fish compensate <amount> [-d "..."] [--yes] [--dry-run] [--rate 5.0] [--batch-id ID]（限频默认 5 req/s，遇 429 调小；--batch-id 用于幂等续跑）
